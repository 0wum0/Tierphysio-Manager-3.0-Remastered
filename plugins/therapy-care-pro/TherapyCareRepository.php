...
class TherapyCareRepository
{
    ...
    public function getAllProgressCategories(): array
    {
        return $this->db->fetchAll(
            "SELECT * FROM `{$this->t('tcp_progress_categories')}` ORDER BY sort_order ASC, name ASC"
        );
    }
    ...
    {
        return $this->db->fetchAll(
            "SELECT * FROM `{$this->t('tcp_progress_categories')}` WHERE is_active = 1 ORDER BY sort_order ASC, name ASC"
        );
    }

    public function findProgressCategoryById(int $id): ?array
    {
        $row = $this->db->fetch("SELECT * FROM `{$this->t('tcp_progress_categories')}` WHERE id = ? LIMIT 1", [$id]);
        return $row ?: null;
    }
    ...
    {
        $this->db->execute(
            "INSERT INTO `{$this->t('tcp_progress_categories')}` (name, description, scale_min, scale_max, scale_label_min, scale_label_max, color, icon, sort_order, is_active)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            [
                $data['name'],
    ...
    {
        $this->db->execute(
            "UPDATE `{$this->t('tcp_progress_categories')}` SET name=?, description=?, scale_min=?, scale_max=?,
    ...
    {
        $this->db->execute("DELETE FROM `{$this->t('tcp_progress_categories')}` WHERE id = ?", [$id]);
    }
    ...
    public function getLatestProgressForPatient(int $patientId): array
    {
        return $this->db->fetchAll(
            "SELECT e.*, c.name AS category_name, c.color AS category_color, c.scale_min, c.scale_max
             ...
             JOIN `{$this->t('tcp_progress_categories')}` c ON c.id = e.category_id
             WHERE e.id IN (
                 SELECT MAX(id) FROM `{$this->t('tcp_progress_entries')}` WHERE patient_id = ? GROUP BY category_id
             )
    ...
    {
        $this->db->execute("DELETE FROM `{$this->t('tcp_progress_entries')}` WHERE id = ?", [$id]);
    }
    ...
    public function getFeedbackForHomework(int $homeworkId): array
    {
        return $this->db->fetchAll(
            "SELECT f.*, o.first_name, o.last_name
             ...
             JOIN `{$this->t('owners')}` o ON o.id = f.owner_id
             WHERE f.homework_id = ?
    ...
    public function getFeedbackForPatient(int $patientId, ?int $days = null): array
    {
        $sql    = 'SELECT f.*, ph.title AS homework_title, o.first_name, o.last_name
                   FROM `'.$this->t('tcp_exercise_feedback').'` f
                   JOIN `'.$this->t('patient_homework').'` ph ON ph.id = f.homework_id
                   JOIN `'.$this->t('owners').'` o ON o.id = f.owner_id
                   WHERE f.patient_id = ?';
        $params = [$patientId];
        if ($days) {
    ...
    {
        $this->db->execute(
            "INSERT INTO `{$this->t('tcp_exercise_feedback')}` (homework_id, patient_id, owner_id, status, comment, feedback_date)
             VALUES (?, ?, ?, ?, ?, ?)",
            [
    ...
    {
        return $this->db->fetchAll("SELECT * FROM `{$this->t('tcp_reminder_templates')}` ORDER BY type ASC, name ASC");
    }

    public function getActiveReminderTemplates(): array
    {
        return $this->db->fetchAll(
            "SELECT * FROM `{$this->t('tcp_reminder_templates')}` WHERE is_active = 1 ORDER BY type ASC"
        );
    }

    public function findReminderTemplateById(int $id): ?array
    {
        $row = $this->db->fetch("SELECT * FROM `{$this->t('tcp_reminder_templates')}` WHERE id = ? LIMIT 1", [$id]);
        return $row ?: null;
    }
    ...
    {
        $this->db->execute(
            "INSERT INTO `{$this->t('tcp_reminder_templates')}` (type, name, subject, body, trigger_hours, is_active)
             VALUES (?, ?, ?, ?, ?, ?)",
            [
                $data['type'],
                $data['name'],
    ...
    {
        $this->db->execute(
            "UPDATE `{$this->t('tcp_reminder_templates')}` SET type=?, name=?, subject=?, body=?, trigger_hours=?, is_active=? WHERE id=?",
            [
                $data['type'],
                $data['name'],
                $data['subject'],
    ...
    {
        $this->db->execute("DELETE FROM `{$this->t('tcp_reminder_templates')}` WHERE id = ?", [$id]);
    }
    ...
    public function getPendingReminderQueue(): array
    {
        return $this->db->fetchAll(
            "SELECT q.*, o.email AS owner_email, o.first_name AS owner_first_name, o.last_name AS owner_last_name,
             ...
             JOIN `{$this->t('owners')}` o ON o.id = q.owner_id
             LEFT JOIN `{$this->t('patients')}` p ON p.id = q.patient_id
    ...
    {
        $this->db->execute(
            "INSERT INTO `{$this->t('tcp_reminder_queue')}` (template_id, type, patient_id, owner_id, appointment_id, subject, body, send_at, status)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending')",
            [
    ...
    {
        $this->db->execute(
            "UPDATE `{$this->t('tcp_reminder_queue')}` SET status='sent', sent_at=NOW() WHERE id=?",
            [$queueId]
        );
    }
    ...
    {
        $this->db->execute(
            "UPDATE `{$this->t('tcp_reminder_queue')}` SET status='failed', error_message=? WHERE id=?",
            [$error, $queueId]
        );
    }
    ...
    {
        return $this->db->fetchAll(
            "SELECT * FROM `{$this->t('tcp_reminder_logs')}` ORDER BY sent_at DESC LIMIT " . (int)$limit
        );
    }
    ...
    {
        $this->db->execute(
            "INSERT INTO `{$this->t('tcp_reminder_logs')}` (queue_id, type, recipient, subject, status, error, sent_at)
             VALUES (?, ?, ?, ?, ?, ?, NOW())",
            [
    ...
    public function reminderAlreadyQueued(int $ownerId, ?int $appointmentId, string $type): bool
    {
        $count = (int)$this->db->fetchColumn(
            "SELECT COUNT(*) FROM `{$this->t('tcp_reminder_queue')}`
    ...
    {
        return $this->db->fetchAll(
            "SELECT * FROM `{$this->t('tcp_reminder_queue')}` WHERE patient_id = ? ORDER BY send_at DESC LIMIT 20",
            [$patientId]
        );
    }
    ...
    {
        $this->db->execute(
            "UPDATE `{$this->t('tcp_reminder_queue')}` SET status='cancelled' WHERE appointment_id=? AND status='pending'",
            [$appointmentId]
        );
    }
    ...
    {
        $this->db->execute("UPDATE `{$this->t('tcp_therapy_reports')}` SET filename=? WHERE id=?", [$filename, $id]);
    }

    public function markTherapyReportSent(int $id, string $sentTo): void
    {
        $this->db->execute(
            "UPDATE `{$this->t('tcp_therapy_reports')}` SET sent_at=NOW(), sent_to=? WHERE id=?",
            [$sentTo, $id]
        );
    }
    ...
    {
        $this->db->execute("DELETE FROM `{$this->t('tcp_therapy_reports')}` WHERE id = ?", [$id]);
    }
    ...
    {
        $sql    = "SELECT e.*, u.name AS created_by_name FROM `{$this->t('tcp_exercise_library')}` e LEFT JOIN `{$this->t('users')}` u ON u.id = e.created_by WHERE e.is_active = 1";
        $params = [];
    ...
    {
        $this->db->execute(
            "UPDATE `{$this->t('tcp_exercise_library')}` SET title=?, category=?, description=?, instructions=?,
    ...
    {
        $this->db->execute("DELETE FROM `{$this->t('tcp_exercise_library')}` WHERE id = ?", [$id]);
    }

    public function getExerciseCategories(): array
    {
        return $this->db->fetchAll(
            "SELECT DISTINCT category FROM `{$this->t('tcp_exercise_library')}` WHERE is_active=1 ORDER BY category ASC"
        );
    }
    ...
    {
        return $this->db->fetchAll(
            "SELECT * FROM `{$this->t('tcp_natural_therapy_types')}` WHERE is_active=1 ORDER BY sort_order ASC, name ASC"
        );
    }
    ...
    {
        $this->db->execute(
            "INSERT INTO `{$this->t('tcp_natural_therapy_types')}` (name, category, description, sort_order, is_active) VALUES (?, ?, ?, ?, ?)",
    ...
    {
        $this->db->execute(
            "UPDATE `{$this->t('tcp_natural_therapy_types')}` SET name=?, category=?, description=?, sort_order=?, is_active=? WHERE id=?",
    ...
    {
        $this->db->execute("DELETE FROM `{$this->t('tcp_natural_therapy_types')}` WHERE id=?", [$id]);
    }
    ...
    public function getNaturalEntriesForPatient(int $patientId): array
    {
        return $this->db->fetchAll(
            "SELECT e.*, t.name AS type_name, t.category AS type_category, u.name AS recorded_by_name
             FROM `{$this->t('tcp_natural_therapy_entries')}` e
             LEFT JOIN `{$this->t('tcp_natural_therapy_types')}` t ON t.id = e.type_id
             LEFT JOIN `{$this->t('users')}` u ON u.id = e.recorded_by
             WHERE e.patient_id = ?
    ...
    {
        $this->db->execute(
            "INSERT INTO `{$this->t('tcp_natural_therapy_entries')}`
    ...
    {
        $this->db->execute(
            "UPDATE `{$this->t('tcp_natural_therapy_entries')}` SET therapy_type=?, agent=?, dosage=?, frequency=?,
    ...
    {
        $this->db->execute("DELETE FROM `{$this->t('tcp_natural_therapy_entries')}` WHERE id=?", [$id]);
    }
    ...
    {
        return $this->db->fetchAll(
            "SELECT m.* FROM `{$this->t('tcp_timeline_meta')}` m
             JOIN `{$this->t('patient_timeline')}` t ON t.id = m.timeline_id
             WHERE t.patient_id = ?",
            [$patientId]
        );
    }
    ...
    {
        $this->db->execute(
            "INSERT INTO `{$this->t('tcp_timeline_meta')}` (timeline_id, event_type, ref_id, ref_table)
             VALUES (?, ?, ?, ?)
    ...
    {
        $this->db->execute(
            "INSERT INTO `{$this->t('tcp_portal_visibility')}` (patient_id, show_progress, show_natural, show_reports)
             VALUES (?, ?, ?, ?)
    ...
    public function getRecentTimeline(int $patientId, int $limit = 5): array
    {
        return $this->db->fetchAll(
            "SELECT t.*, m.event_type AS tcp_event_type, m.icon AS tcp_icon, m.badge_color AS tcp_badge_color
             ...
             LEFT JOIN `{$this->t('tcp_timeline_meta')}` m ON m.timeline_id = t.id
             WHERE t.patient_id = ?
        ...
        try {
            return $this->db->fetchAll(
                "SELECT a.id, a.title, a.start_at, a.end_at, a.status, o.first_name, o.last_name, o.email
                 ...
                 JOIN `{$this->t('patients')}` p ON p.id = a.patient_id
                 JOIN `{$this->t('owners')}` o ON o.id = p.owner_id
    ...
    {
        return $this->db->fetchAll(
            "SELECT * FROM `{$this->t('patient_homework')}` WHERE patient_id = ? ORDER BY created_at DESC",
            [$patientId]
        );
    }
    ...
    {
        $totalEntries = (int)$this->db->fetchColumn("SELECT COUNT(*) FROM `{$this->t('tcp_progress_entries')}`");
        $totalFeedback = (int)$this->db->fetchColumn("SELECT COUNT(*) FROM `{$this->t('tcp_exercise_feedback')}`");
        $painFeedback  = (int)$this->db->fetchColumn("SELECT COUNT(*) FROM `{$this->t('tcp_exercise_feedback')}` WHERE status='pain' AND feedback_date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)");
        $pendingReminders = (int)$this->db->fetchColumn("SELECT COUNT(*) FROM `{$this->t('tcp_reminder_queue')}` WHERE status='pending'");
        $reports       = (int)$this->db->fetchColumn("SELECT COUNT(*) FROM `{$this->t('tcp_therapy_reports')}`");
