...
-- Alle Tabellen sind additiv; bestehende Tabellen werden nicht modifiziert.
--
-- Placeholder-Konvention: {{PREFIX}} wird zur Laufzeit durch den Tenant-Prefix ersetzt
-- (z.B. "t_peters_physio_4ffe0d_") via ServiceProvider::runMigrations().
-- Das gilt für TABELLENNAMEN UND Foreign-Key-Constraint-Namen, weil MySQL
-- FK-Constraint-Namen pro Schema (Datenbank) eindeutig verlangt.
-- Ohne diesen Prefix auf Constraint-Namen würden mehrere Tenants bei der Migration
-- mit "Duplicate foreign key constraint name" crashen.

-- ══════════════════════════════════════════════════════════
-- MODULE 1: THERAPY PROGRESS TRACKING
-- ══════════════════════════════════════════════════════════

CREATE TABLE IF NOT EXISTS `{{PREFIX}}tcp_progress_categories` (
...
CREATE TABLE IF NOT EXISTS `{{PREFIX}}tcp_progress_entries` (
    `id`              INT UNSIGNED NOT NULL AUTO_INCREMENT,
    ...
    CONSTRAINT `{{PREFIX}}fk_tcp_pe_patient`  FOREIGN KEY (`patient_id`)  REFERENCES `{{PREFIX}}patients` (`id`) ON DELETE CASCADE,
    CONSTRAINT `{{PREFIX}}fk_tcp_pe_category` FOREIGN KEY (`category_id`) REFERENCES `{{PREFIX}}tcp_progress_categories` (`id`) ON DELETE CASCADE,
    CONSTRAINT `{{PREFIX}}fk_tcp_pe_user`     FOREIGN KEY (`recorded_by`) REFERENCES `{{PREFIX}}users` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

INSERT IGNORE INTO `{{PREFIX}}tcp_progress_categories` (`id`, `name`, `description`, `scale_min`, `scale_max`, `scale_label_min`, `scale_label_max`, `color`, `sort_order`) VALUES
...
CREATE TABLE IF NOT EXISTS `{{PREFIX}}tcp_exercise_feedback` (
    `id`            INT UNSIGNED NOT NULL AUTO_INCREMENT,
    ...
    CONSTRAINT `{{PREFIX}}fk_tcp_ef_patient` FOREIGN KEY (`patient_id`) REFERENCES `{{PREFIX}}patients` (`id`) ON DELETE CASCADE,
    CONSTRAINT `{{PREFIX}}fk_tcp_ef_owner`   FOREIGN KEY (`owner_id`)   REFERENCES `{{PREFIX}}owners` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `{{PREFIX}}tcp_reminder_templates` (
    `id`            INT UNSIGNED NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `{{PREFIX}}tcp_reminder_queue` (
    `id`             INT UNSIGNED NOT NULL AUTO_INCREMENT,
    ...
    CONSTRAINT `{{PREFIX}}fk_tcp_rq_patient` FOREIGN KEY (`patient_id`) REFERENCES `{{PREFIX}}patients` (`id`) ON DELETE CASCADE,
    CONSTRAINT `{{PREFIX}}fk_tcp_rq_owner`   FOREIGN KEY (`owner_id`)   REFERENCES `{{PREFIX}}owners` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `{{PREFIX}}tcp_reminder_logs` (
    `id`        INT UNSIGNED NOT NULL AUTO_INCREMENT,
...
INSERT IGNORE INTO `{{PREFIX}}tcp_reminder_templates` (`id`, `type`, `name`, `subject`, `body`, `trigger_hours`, `is_active`) VALUES
...
CREATE TABLE IF NOT EXISTS `{{PREFIX}}tcp_therapy_reports` (
    `id`                      INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `patient_id`              INT UNSIGNED NOT NULL,
    ...
    CONSTRAINT `{{PREFIX}}fk_tcp_tr_patient` FOREIGN KEY (`patient_id`) REFERENCES `{{PREFIX}}patients` (`id`) ON DELETE CASCADE,
    CONSTRAINT `{{PREFIX}}fk_tcp_tr_user`    FOREIGN KEY (`created_by`) REFERENCES `{{PREFIX}}users`    (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `{{PREFIX}}tcp_exercise_library` (
    `id`                INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `title`             VARCHAR(255) NOT NULL,
    ...
    `species_tags`      VARCHAR(255) NULL,
    `therapy_tags`      VARCHAR(255) NULL,
    ...
    CONSTRAINT `{{PREFIX}}fk_tcp_el_user` FOREIGN KEY (`created_by`) REFERENCES `{{PREFIX}}users` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `{{PREFIX}}tcp_natural_therapy_types` (
    `id`          INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `name`        VARCHAR(100) NOT NULL,
    `category`    VARCHAR(100) NOT NULL DEFAULT 'sonstiges',
    `description` TEXT NULL,
...
CREATE TABLE IF NOT EXISTS `{{PREFIX}}tcp_natural_therapy_entries` (
    `id`             INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `patient_id`     INT UNSIGNED NOT NULL,
    ...
    CONSTRAINT `{{PREFIX}}fk_tcp_nte_patient` FOREIGN KEY (`patient_id`)  REFERENCES `{{PREFIX}}patients` (`id`) ON DELETE CASCADE,
    CONSTRAINT `{{PREFIX}}fk_tcp_nte_type`    FOREIGN KEY (`type_id`)     REFERENCES `{{PREFIX}}tcp_natural_therapy_types` (`id`) ON DELETE SET NULL,
    CONSTRAINT `{{PREFIX}}fk_tcp_nte_user`    FOREIGN KEY (`recorded_by`) REFERENCES `{{PREFIX}}users` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

INSERT IGNORE INTO `{{PREFIX}}tcp_natural_therapy_types` (`id`, `name`, `category`, `sort_order`) VALUES
...
CREATE TABLE IF NOT EXISTS `{{PREFIX}}tcp_timeline_meta` (
...
CREATE TABLE IF NOT EXISTS `{{PREFIX}}tcp_portal_visibility` (
    `patient_id`    INT UNSIGNED NOT NULL,
    ...
    CONSTRAINT `{{PREFIX}}fk_tcp_pv_patient` FOREIGN KEY (`patient_id`) REFERENCES `{{PREFIX}}patients` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
